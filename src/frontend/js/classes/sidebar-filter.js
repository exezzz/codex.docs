...
export default class SidebarFilter {
  ...
  init(sections, sidebarContent, search, setSectionCollapsed) {
    ...
    window.__SIDEBAR_FILTER__ = this;
    ...
    this.fuse = new Fuse(this.pages, {
      keys: ["title", "content"],
      threshold: 0.3, // 0.0 = только точный, 1.0 = всё подряд
      ignoreLocation: true, // не привязываться к позиции
      includeScore: true,
    });
  ...
  performSearch(query) {
    ...
    this.sections.forEach((sec) => {
      sec.classList.remove(SidebarFilter.CSS.sectionHidden);
      sec
        .querySelectorAll("li[data-id]")
        .forEach((li) =>
          li.classList.remove(SidebarFilter.CSS.sectionListItemWrapperHidden)
        );
    });
    if (!query) return;
